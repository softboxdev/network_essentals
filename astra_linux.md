...
sequenceDiagram
    ...
    participant ALD as ALD-контроллер<br/>Astra Linux
    participant AD as Microsoft AD
    participant S as Samba Server
    participant C as Клиент Windows

    Note over A, C: ЭТАП 1: Развертывание базовой инфраструктуры
    
    A->>ALD: Установка astra-domain-admin
    ALD-->>A: Пакеты установлены
    A->>ALD: ald-provision (создание домена)
    ALD-->>A: Домен astra.local создан
    
    A->>AD: Создание пользователя доверия ald_trust
    AD-->>A: Пользователь создан
    
    A->>S: Установка Samba
    S-->>A: Samba установлен
    A->>S: Настройка smb.conf
    S-->>A: Конфигурация применена
    
    Note over A, C: ЭТАП 2: Настройка интеграции
    
    A->>S: net ads join -U administrator
    S->>AD: Запрос на присоединение к домену
    AD-->>S: Подтверждение + компьютерная запись
    S-->>A: Успешно присоединен к домену
    
    A->>ALD: Настройка krb5.conf (доверие)
    A->>AD: Настройка conditional forwarders DNS
```
...
```mermaid
sequenceDiagram
    participant U as Пользователь AD
    participant C as Клиент Windows
    participant AD as Microsoft AD
    participant S as Samba Server<br/>Astra Linux
    participant ALD as ALD-контроллер
    participant PAM as PAM-модуль

    Note over U, PAM: СЦЕНАРИЙ: Вход пользователя AD на Linux-сервер
    
    U->>C: Логин в Windows (company.local)
    C->>AD: Аутентификация Kerberos
    AD-->>C: TGT-билет получен
    
    U->>S: Попытка доступа к SSH/сервису
    S->>PAM: Запрос аутентификации
    PAM->>S: winbind проверка учетных данных
    
    S->>AD: Запрос проверки пользователя
    AD-->>S: Подтверждение учетных данных
    
    S->>ALD: Запрос дополнительных прав (опционально)
    ALD-->>S: Информация о правах
    
    S-->>U: Успешный вход!<br/>Сессия создана
```
...
sequenceDiagram
    ...
    Note over U, FS: СЦЕНАРИЙ: Доступ к общей папке Samba
    
    U->>C: \\samba-server\shared
    C->>AD: Запрос Kerberos-билета для CIFS
    AD-->>C: Service ticket для Samba
    
    C->>S: SMB-запрос с Kerberos-билетом
    S->>AD: Проверка билета
    AD-->>S: Билет валиден
    
    S->>FS: Проверка прав доступа к /srv/samba/shared
    FS-->>S: Права подтверждены
    
    S->>S: Сопоставление SID↔UID (idmap)
    S-->>C: Доступ к папке разрешен
    C-->>U: Открыта общая папка
```
...
sequenceDiagram
    ...
    Note over A, S: ЭТАП 3: Миграция пользователей
    
    A->>PS: Get-ADUser -Filter *
    PS->>AD: Экспорт данных пользователей
    AD-->>PS: CSV с пользователями
    PS-->>A: Файл users.csv
    
    loop Для каждого пользователя
        A->>ALD: ipa user-add --first=Имя --last=Фамилия
        ALD-->>A: Пользователь создан
        A->>ALD: Установка временного пароля
        ALD-->>A: Пароль установлен
    end
    
    Note over A, S: Параллельная работа доменов
    A->>S: Настройка прав для mixed-пользователей
    S-->>A: Права настроены
    
    Note over A, S: Постеенный переход
    A->>AD: Отключение старых учетных записей
    AD-->>A: Учетные записи отключены
```
...
sequenceDiagram
    ...
    participant F как Forwarder

    Note over C, F: КРИТИЧЕСКИЙ КОМПОНЕНТ: DNS-интеграция
    
    C->>ALD: Запрос company.local
    ALT Conditional Forwarding настроен
        ALD->>AD: Пересылка запроса
        AD-->>ALD: DNS-ответ
        ALD-->>C: Возврат IP-адресов
    ELSE Forwarding не настроен
        ALD->>F: Стандартный forwarder (8.8.8.8)
        F-->>ALD: NXDOMAIN (домен не найден)
        ALD-->>C: Ошибка разрешения
    end
    
    C->>AD: Запрос astra.local
    ALT Conditional Forwarding настроен
        AD->>ALD: Пересылка запроса
        ALD-->>AD: DNS-ответ
        AD-->>C: Возврат IP-адресов
    ELSE Forwarding не настроен
        AD->>F: Стандартный forwarder
        F-->>AD: NXDOMAIN
        AD-->>C: Ошибка разрешения
    end
```
...
sequenceDiagram
    ...
    participant C as Рабочая станция
    participant AD as Microsoft AD
    participant S as Samba/ALD сервис
    participant K как Kerberos

    Note over U, K: ИДЕАЛЬНАЯ СХЕМА: Единый вход
    
    U->>C: Вход в Windows-домен
    C->>AD: Запрос TGT
    AD-->>C: Kerberos TGT получен
    
    U->>C: Доступ к веб-сервису на ALD
    C->>K: Запрос service ticket для HTTP/service.astra.local
    K->>AD: Проверка доверия между REALMS
    AD-->>K: Доверие подтверждено
    K-->>C: Service ticket выдан
    
    C->>S: Запрос к сервису с Kerberos-билетом
    S->>K: Проверка билета
    K-->>S: Билет валиден
    S-->>U: Доступ к сервису предоставлен<br/>(БЕЗ ввода пароля!)
```
